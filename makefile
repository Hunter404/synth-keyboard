...
LIBS_LOCAL_BASE  := lib
LIBS_LOCAL       := Adafruit NeoTrellis M4 Library 
...
DEFINES     += -DF_CPU=180000000 -DUSB_SERIAL -DLAYOUT_SWEDISH
...
UPL_PJRC    := "$(UPL_PJRC_B)/teensy_post_compile" -test -file=$(TARGET_NAME) -path=$(BIN) -tools="$(UPL_PJRC_B)" -board=$(BOARD_ID) -reboot -v
